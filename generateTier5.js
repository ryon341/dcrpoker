...
function chooseCarryoverQuestions(tier4Questions) {
  if (!Array.isArray(tier4Questions)) {
    throw new Error('tier4Questions.json is not a valid array.');
  }

  const actionQs = tier4Questions.filter(q => q.category === 'action');
  const outsQs = tier4Questions.filter(q => q.category === 'outs');
  const evQs = tier4Questions.filter(q => q.category === 'ev');
  ...
  return shuffle(selected).map((q, idx) => {
    ...
    return {
      ...copied,
      id: `t5_carry_${String(idx + 1).padStart(3, '0')}`,
      tier,
      tierIndex,
      tags: Array.from(new Set([...(copied.tags || []), 'carryover', 'from_tier_4'])),
      difficultyScore: Math.min(100, Number(copied.difficultyScore || 20) + 8),
    };
  });
}
...
function makeActionQuestion(index, level) {
  ...
  const openHandsByLevel = {
    ...
    5: ['22', 'A4s', 'A3s', '76s', '65s', '54s', 'KTo', 'QTo', 'J9s'],
  };
  ...
  const defendHandsByLevel = {
    ...
    5: ['33', '22', '87s', '76s', '65s', '54s', 'KTo', 'QJo'],
  };
  ...
  if (scenario === 'folds_to_you') {
    ...
    if (
      ...
      ['KTo', 'QTo', '22', '33', 'J9s', '54s'].includes(hand)
    ) {
      correctAction = 'FOLD';
      explanation = `${hand} is too weak to open from ${pos} in this Master-tier spot.`;
    } else {
      correctAction = 'RAISE';
      explanation = `${hand} is strong enough to open from ${pos}, especially with position or sufficient hand strength at ${stack}BB.`;
    }
    ...
    if (['AA', 'KK', 'QQ', 'JJ', 'AKs', 'AKo'].includes(hand)) {
      correctAction = 'RAISE';
      explanation = `${hand} is premium and should usually continue aggressively against an open.`;
    } else if (
      ['TT', '99', '88', 'AQs', 'AJs', 'ATs', 'KQs', 'AQo', 'AJo', 'KQo', 'QJs', 'JTs', 'T9s'].includes(hand) &&
      ['CO', 'BTN', 'SB', 'BB'].includes(pos)
    ) {
      correctAction = 'CALL';
      explanation = `${hand} is strong enough to continue, but flatting is the preferred simplified option here.`;
    } else {
      correctAction = 'FOLD';
      explanation = `${hand} is too marginal versus an open from ${raiserPos} in this spot.`;
    }
    ...
    hand = random(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', 'AKs', 'AQs', 'AJs', 'AKo', 'AQo', 'KQs', 'QJs']);
    ...
    if (['AA', 'KK', 'QQ', 'AKs', 'AKo'].includes(hand)) {
      ...
      correctAction = 'CALL';
      explanation = `${hand} is often good enough to continue, but not always strong enough to escalate further in this simplified model.`;
    } else {
      correctAction = 'FOLD';
    ...
    heroPosition = heroOpenPos;
  } else if (scenario === 'blind_vs_blind') {
    const heroPos = random(['SB', 'BB']);
    heroPosition = heroPos;
    hand = random(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', '66', 'AJs', 'ATs', 'A9s', 'KQs', 'KJs', 'QJs', 'JTs', 'T9s', '98s', '87s', '76s']);
    ...
    if (heroPos === 'SB') {
      ...
      } else if (['66', 'JTs', 'T9s', '98s', '87s'].includes(hand)) {
        correctAction = 'CALL';
        explanation = `${hand} is playable in this simplified blind-versus-blind model due to playability and realization.`;
      } else {
        correctAction = 'FOLD';
        explanation = `${hand} is too weak to continue from the small blind in this spot.`;
      }

      prompt = `Action folds to you in the small blind. You hold ${hand} with ${stack}BB effective. What is the best action?`;
    } else {
      if (['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', '66', 'AJs', 'ATs', 'A9s', 'KQs', 'KJs', 'QJs'].includes(hand)) {
        correctAction = 'RAISE';
        explanation = `${hand} is strong enough to continue aggressively in a blind-versus-blind battle.`;
      } else if (['JTs', 'T9s', '98s', '87s', '76s'].includes(hand)) {
        correctAction = 'CALL';
        explanation = `${hand} can continue due to strong playability in this simplified blind battle.`;
      } else {
        correctAction = 'FOLD';
    ...
    hand = random(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', 'AKs', 'AQs', 'AJs', 'KQs', 'QJs', 'JTs', 'AQo']);
    ...
    } else if (['TT', '99', '88', '77', 'AJs', 'KQs', 'QJs'].includes(hand)) {
      correctAction = 'CALL';
      explanation = `${hand} can continue, but calling is the preferred simplified option in this spot.`;
    } else {
      correctAction = 'FOLD';
      explanation = `${hand} is too marginal for this squeeze configuration.`;
    }
  ...
  } else if (scenario === 'iso_raise_spot') {
    hand = random(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', 'AJs', 'ATs', 'A9s', 'KQs', 'KJs', 'QJs', 'JTs', 'T9s', '98s']);
    const limpers = random([1, 2, 3]);
    const limperText =
      limpers === 1 ? 'One player limps' :
      limpers === 2 ? 'Two players limp' :
      'Three players limp';

    if (['AA', 'KK', 'QQ', 'JJ', 'TT', '99', 'AJs', 'ATs', 'A9s', 'KQs', 'KJs'].includes(hand)) {
      correctAction = 'RAISE';
      explanation = `${hand} is strong enough to isolate limpers and take initiative.`;
    } else if (['88', '77', 'QJs', 'JTs', 'T9s', '98s'].includes(hand) && ['CO', 'BTN'].includes(pos)) {
      correctAction = 'CALL';
      explanation = `${hand} is playable but not always a mandatory isolation raise in this simplified model.`;
    } else {
      correctAction = 'FOLD';
      explanation = `${hand} is too weak or too marginal to continue profitably from ${pos} against multiple limpers.`;
    }
    ...
    hand = random(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', '66', 'AJs', 'ATs', 'A9s', 'A8s', 'KQs', 'KJs', 'QJs', 'JTs', 'T9s', '98s', '87s']);
    const openerPos = random(['CO', 'BTN']);
    const heroPos = openerPos === 'CO' ? 'BTN' : random(['SB', 'BB']);
    heroPosition = heroPos;

    if (['AA', 'KK', 'QQ', 'JJ', 'TT', '99', 'AJs', 'ATs', 'A9s', 'KQs', 'KJs'].includes(hand)) {
      correctAction = 'RAISE';
      explanation = `${hand} is strong enough to apply late-position pressure against a wide opening range.`;
    } else if (['88', '77', '66', 'QJs', 'JTs', 'T9s', '98s'].includes(hand)) {
      correctAction = 'CALL';
      explanation = `${hand} can continue profitably, but calling is the more controlled simplified option here.`;
    } else {
      correctAction = 'FOLD';
      explanation = `${hand} is too weak to continue profitably versus late-position pressure in this simplified model.`;
    }

    prompt = `${openerPos} opens wide. You are in ${heroPos} with ${hand} and ${stack}BB effective. What is the best action?`;
    tags.push(hand, 'late_position_pressure');
  }

  return {
    id: `t5_new_${String(index).padStart(3, '0')}`,
    tier,
    tierIndex,
    level,
    category: 'action',
    ...
    difficultyScore: difficultyForLevel(level, Math.floor(Math.random() * 10) + 9),
  };
}
...
function makeEvQuestion(index, level) {
  const scenariosByLevel = {
    1: [
      { pot: 90, call: 30, equity: 26 },
      { pot: 100, call: 35, equity: 29 },
      { pot: 110, call: 40, equity: 24 },
    ],
    2: [
      { pot: 120, call: 40, equity: 27 },
      { pot: 140, call: 50, equity: 31 },
      { pot: 150, call: 55, equity: 25 },
    ],
    3: [
      { pot: 170, call: 60, equity: 28 },
      { pot: 180, call: 65, equity: 30 },
      { pot: 200, call: 70, equity: 24 },
    ],
    4: [
      { pot: 220, call: 75, equity: 27 },
      { pot: 240, call: 80, equity: 29 },
      { pot: 260, call: 90, equity: 32 },
    ],
    5: [
      { pot: 280, call: 95, equity: 24 },
      { pot: 300, call: 100, equity: 28 },
      { pot: 320, call: 110, equity: 31 },
    ],
  };
...
function generateNewTier5Questions() {
  const generated = [];
...
function interleaveQuestions(carryover, fresh) {
  ...
  return result.slice(0, TOTAL_QUESTIONS).map((q, idx) => {
    ...
    return {
      ...q,
      level,
      difficultyScore: Math.max(Number(q.difficultyScore || 20), difficultyForLevel(level, 4)),
    };
  });
}

function main() {
  const tier4Path = path.join(process.cwd(), 'tier4Questions.json');

  if (!fs.existsSync(tier4Path)) {
    throw new Error(`Missing tier4Questions.json in ${process.cwd()}. Put Tier 4 output in the same folder first.`);
  }

  const tier4Questions = JSON.parse(fs.readFileSync(tier4Path, 'utf8'));
  const carryover = chooseCarryoverQuestions(tier4Questions);
  const fresh = generateNewTier5Questions();
  ...
  const jsonPath = path.join(process.cwd(), 'tier5Questions.json');
  fs.writeFileSync(jsonPath, JSON.stringify(finalQuestions, null, 2), 'utf8');

  const tsPath = path.join(process.cwd(), 'tier5Questions.ts');
  const tsContent = `export const tier5Questions = ${JSON.stringify(finalQuestions, null, 2)} as const;\n`;
  fs.writeFileSync(tsPath, tsContent, 'utf8');

  console.log(`✅ Generated ${finalQuestions.length} Tier 5 questions`);
  console.log(`📄 JSON: ${jsonPath}`);
  console.log(`📄 TS:   ${tsPath}`);
  console.log(`🔁 Carryover from Tier 4: ${CARRYOVER_COUNT}`);
  console.log(`🆕 New Tier 5 questions: ${NEW_COUNT}`);
}
